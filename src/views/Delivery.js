...
class Delivery extends React.Component {
    ...
    selectGoods(v){
        ...
        this.setState({nameValue:v,maxNumber:goodsObj.number,selectedGoods:goodsObj},()=>{
    ...
    sureStorage(){
        if(this.state.nameValue.length&&this.state.priceValue&&this.state.storageValue){
            ...
            console.log(data);
            Toast.loading('正在出库', 0);
            $.post($.baseUrl+'houserecord/add',data,(res)=>{
                console.log(res);
                if(res.n&&res.ok){
                    this.setState({nameValue:[]});
                    Toast.success('出库成功', 1.5);
                }else{
                    Toast.fail('出库失败', 1.5)
                }
            })
        ...
        return (
            <div>
                <NavBar
                    ...
                    rightContent={
                        <Link to={{pathname:'/home/record',state:{queryType:'出库'}}} style={{color:'#fff'}}>出库记录</Link>
                    }
                >出库</NavBar>
                <List>
                    ...
                    <Picker data={this.state.friendsData} cols={1} value={this.state.friendValue} onChange={this.selectFriend.bind(this)} className="forss">
                        <List.Item arrow="horizontal">选择出货商：</List.Item>
                    </Picker>
                    <Item extra={
                    ...
                    <Item extra={
                        <Stepper style={{ width: '100%', minWidth: '120px' }} min={0} max={this.state.maxNumber} showNumber size="small"
                ...
                <WingBlank>
                    <WhiteSpace />
                    <Button type="primary" onClick={this.sureStorage.bind(this)}>确定出库</Button>
                </WingBlank>
